(function (root) {
  ...
  function activeDays(data) {
    var directKeys = ["activeDays", "daysActive", "diasAtivos"];
    for (var i = 0; i < directKeys.length; i++) {
      var direct = metricNumber(data && data[directKeys[i]]);
      if (direct != null) return { value: Math.max(0, Math.round(direct)), referenceDate: data.adsUpdatedAt || data.adsLibraryCheckedAt || "", source: "current" };
    }

    var topAds = Array.isArray(data && data.brandTopAds) ? data.brandTopAds : [];
    var known = topAds.map(function (item) { return metricNumber(item && (item.daysActive != null ? item.daysActive : item.diasAtivos)); }).filter(function (value) { return value != null; });
    if (known.length) return { value: Math.max.apply(Math, known), referenceDate: data.adsUpdatedAt || data.adsLibraryCheckedAt || "", source: "top_ads" };
    ...
    var firstPositive = history.find(function (row) { return row.value > 0; });
    var start = dateKey(data && (data.adsStartedAt || data.startDate || data.startedAt)) || firstPositive && firstPositive.date || "";
    var end = history.length ? history[history.length - 1].date : dateKey(data && (data.adsUpdatedAt || data.adsLibraryCheckedAt)) || dateKey(new Date());
    if (!start || !end) return { value: null, referenceDate: end, source: "missing" };
    var startDate = new Date(start + "T12:00:00Z"), endDate = new Date(end + "T12:00:00Z");
    if (!Number.isFinite(startDate.getTime()) || !Number.isFinite(endDate.getTime()) || endDate < startDate) return { value: null, referenceDate: end, source: "missing" };
    return { value: Math.floor((endDate - startDate) / 86400000) + 1, referenceDate: end, source: "dates" };
  }

  function offerMetric(data, sort) {
    if (sort === "active_ads") return activeAds(data);
    if (sort === "active_days") return activeDays(data);
  ...
  root.SwipeMetrics = Object.freeze({ metricNumber: metricNumber, dailySpend: dailySpend, spendWindow: spendWindow, activeAds: activeAds, activeDays: activeDays, offerMetric: offerMetric, compareOffers: compareOffers });
